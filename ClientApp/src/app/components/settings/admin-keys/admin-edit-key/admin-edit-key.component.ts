...
@Component({
  ...
  styleUrls: ['./admin-edit-key.component.css']
})
...
export class AdminEditKeyComponent  {
  ...
  constructor(public dialogRef: MatDialogRef<AdminKeysComponent>,
    @Inject(MAT_DIALOG_DATA) public data: any,  public httpclient: HttpClient, @Inject('BASE_URL') public baseUrl: string, public settingsService: SettingsService){
      if (data){
        this.key = data;
        this.editMode = true;
      }
